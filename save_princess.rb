class SavePrincess
 ...
 class << self
   def start_rescue
     puts "Please enter a grid size"
     size_info = gets.chomp
     grid_size = size_info.to_i
     if grid_size >= GRID_SIZE_LOWER_LIMIT and grid_size < GRID_SIZE_UPPER_LIMIT 
       puts "Entered grid size #{grid_size}"
       grid = get_grid_element(grid_size)
       display_path_to_princess(grid_size, grid)
     else
       puts "Entered grid size #{size_info}. Please make sure that grid size is between #{GRID_SIZE_LOWER_LIMIT} and #{GRID_SIZE_UPPER_LIMIT}"
     end  
   end  
   ...
   def get_grid_element(grid_size)
     ...
     0.upto grid_row_count do |row|  
       ...
       puts " You have entered the first row #{grid_col.inspect}"
       grid[row] = grid_col
     end
     puts "You have entered the grid #{grid.inspect}"
     grid
   end
   ...
   def display_path_to_princess(grid_size, grid)
    ...
    if bot_row
      ...
      puts "Hurray!!! I found the bot position at #{bot_position.inspect} and now the mission start to find the location for Princess"
      ...
      if princess_row
        ...
        puts "Yes i did it located princess position at #{princess_position.inspect} and now the next job starts"
